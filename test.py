...
class LinearRegressionTestCase(unittest.TestCase):
    ...
    def test_matxRound(self):
        ...
        for decpts in range(10):
            ...
            mat = matrix.tolist()
            dec_true = [[Decimal(str(round(num, decpts)))
                         for num in row] for row in mat]
    ...
    def test_transpose(self):
        for _ in range(100):
            ...
            self.assertEqual(
                t.shape, (c, r), "Expected shape{}, but got shape{}".format((c, r), t.shape))
        ...
        for _ in range(9999):
            ...
            if np.linalg.matrix_rank(A) < r:
                ...
                self.assertEqual(np.array(
                    x).shape, (r, 1), "Expected shape({},1), but got shape{}".format(r, np.array(x).shape))
